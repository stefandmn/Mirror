...
VNCServer::VNCServer(int BPP, int frameRate)
{
	this->BPP = BPP;
...
void VNCServer::Open()
{
	...
	/* DispmanX expects buffer rows to be aligned to a 32 bit boundaries */
...
bool VNCServer::IsOpen()
{
	return m_display.IsOpen();
}

void VNCServer::Run(int port, const std::string& password,
...
*/
double VNCServer::TimeToTakePicture() 
{
	double now = getTime();
...
*/
int VNCServer::TakePicture(unsigned char *buffer)
{
...
void VNCServer::DoPtr(int buttonMask, int x, int y, rfbClientPtr cl)
{
	auto server = reinterpret_cast<VNCServer *>(cl->screen->screenData);
...
void VNCServer::DoKey(rfbBool down, rfbKeySym key, rfbClientPtr cl)
{
	((VNCServer *)(cl->screen->screenData))->m_keyboard.DoKey(down, key);
}

enum rfbNewClientAction VNCServer::newclient(rfbClientPtr cl)
{
	return ((VNCServer *)(cl->screen->screenData))->NewClient(cl);
}
...
{
	((VNCServer *)(cl->screen->screenData))->ClientGone(cl);
}
